    ...
    <div onClick={handleClick} className="cursor-pointer">
      <Card hover className="h-full overflow-hidden group">
        ...
        <div className="relative aspect-square md:aspect-[4/3] bg-gray-100 overflow-hidden">
          ...
          {listing.promotionStatus === 'ACTIVE' && (
            <div className="absolute top-2 left-2 md:top-3 md:left-3">
              <Badge className="bg-primary text-white shadow-lg shadow-primary/40 border-0 font-semibold px-1.5 py-0.5 md:px-3 md:py-1.5 flex items-center text-[10px] md:text-xs">
                <svg className="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12l2 2 4-4m5.618-4.016A11.955 11.955 0 0112 2.944a11.955 11.955 0 01-8.618 3.04A12.02 12.02 0 003 9c0 5.591 3.824 10.29 9 11.622 5.176-1.332 9-6.03 9-11.622 0-1.042-.133-2.052-.382-3.016z" />
                </svg>
                <span className="ml-1">Promosi</span>
              </Badge>
            </div>
          )}
          ...
          <div className="absolute bottom-2 left-2 md:bottom-3 md:left-3">
            <Badge className={`${categoryType === 'JASA' 
              ? 'bg-primary text-white' 
              : 'bg-amber-500 text-white'} shadow-lg shadow-black/10 border-0 font-semibold px-2 py-0.5 md:px-3 md:py-1.5 flex items-center gap-1`}>
              {categoryType === 'JASA' ? (
                <svg className="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <CardContent className="p-2 md:p-4">
          ...
          {listing.userId && (
            <Link 
              href={`/provider/${listing.userId}`}
              onClick={(e) => e.stopPropagation()}
              className="mt-2 flex items-center justify-center gap-2 text-xs text-primary hover:text-primary-dark transition-colors"
            >
              <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z" />
              </svg>
              <span>Lihat profil penyedia</span>
            </Link>
          )}
        </CardContent>
      </Card>
