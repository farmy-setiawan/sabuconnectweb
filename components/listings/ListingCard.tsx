        ...
        <div className="relative aspect-square md:aspect-[4/3] bg-gray-100 overflow-hidden">
          ...
          {listing.promotionStatus === 'ACTIVE' && (
            <div className="absolute top-2 left-2 md:top-3 md:left-3">
              <Badge className="bg-gradient-to-r from-green-600 to-green-500 text-white shadow-lg shadow-green-600/40 border-0 font-semibold px-1.5 py-0.5 md:px-3 md:py-1.5 flex items-center">
                <svg className="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12l2 2 4-4m5.618-4.016A11.955 11.955 0 0112 2.944a11.955 11.955 0 01-8.618 3.04A12.02 12.02 0 003 9c0 5.591 3.824 10.29 9 11.622 5.176-1.332 9-6.03 9-11.622 0-1.042-.133-2.052-.382-3.016z" />
                </svg>
                <span className="hidden md:inline text-[10px] md:text-xs">Terpercaya</span>
              </Badge>
            </div>
          )}
          ...
          {listing.user?.isVerified && (
            <div className="absolute top-2 right-2 md:top-3 md:right-3">
              <Badge className="bg-gradient-to-r from-blue-500 to-blue-600 text-white shadow-lg shadow-blue-500/30 border-0 flex items-center gap-1 px-1.5 py-0.5 md:px-3 md:py-1.5">
                ...
                <span className="text-[10px] md:text-xs hidden md:inline">Terverifikasi</span>
              </Badge>
            </div>
          )}
        ...
        <CardContent className="p-2 md:p-4">
          ...
          <div className="flex items-center gap-4 mb-2 hidden md:flex">
            <span className="flex items-center gap-1 text-xs text-gray-500">
              ...
              {typeof listing.views === 'number' ? listing.views.toLocaleString('id-ID') : listing.views}
            </span>
          </div>
          ...
          {isProduct ? (
            ...
            hasWhatsApp ? (
              <Button variant="primary" className="w-full h-9 md:h-10 text-xs md:text-sm" size="sm" onClick={handleWhatsAppClick}>
            ...
            hasWhatsApp ? (
              <Button variant="secondary" className="w-full h-9 md:h-10 text-xs md:text-sm" size="sm" onClick={handleWhatsAppClick}>
                <svg className="w-3 h-3 md:w-4 md:h-4 mr-1 md:mr-2" fill="currentColor" viewBox="0 0 24 24">
                  <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z"/>
                </svg>
